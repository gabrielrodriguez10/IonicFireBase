...
export class CargaArchivoProvider {
  ...
  private cargar_ultimo_key(){
    return this.afDB.list('/post', ref=> ref.orderByKey().limitToLast(1) ).valueChanges().map( (post:any) =>{
      console.log(post);
  ...
  cargar_imagen_firebase( archivo: ArchivoSubir){
    let promesa = new Promise( (resolve, reject)=>{
      ...
      uploadTask.on( firebase.storage.TaskEvent.STATE_CHANGED, ()=>{ }, // saber el % de cuantos Mbs se han subido
        ( error ) =>{
